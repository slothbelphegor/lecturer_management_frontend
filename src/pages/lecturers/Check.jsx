...
const CheckLecturer = () => {
  ...
  const submissionPromote = (data) => {
    ...
    AxiosInstance.post(`lecturers/${lecturer_id}/sign_contract/`, sentData)
      ...
      .catch((error) => {
        setMessage("An error occurred while updating the lecturer.");
        setIsError(true);
        console.error(error.response.data);
      })
      .finally(() => {
  ...
  return (
    <div>
      ...
      <LecturerInfoForm lecturer={currentLecturer} 
        onAccept={submissionAccept} onReject={submissionReject} onPromote={submissionPromote}
        readOnly={true} isChecking={true}/>
      {showMessage ? (
        <MyMessage
          text={message}
